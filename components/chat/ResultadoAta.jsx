function ItemAcao({ item }) {
  ...
  return (
    <li className="font-mono text-xs text-preto/50">{JSON.stringify(item)}</li>
  );
}
...
export default function ResultadoAta({ ata, lacunas }) {
  ...
  const participantesNomeados = ata.participantes?.filter((p) => p.nome) ?? [];
      ...
      <div className="mt-5 space-y-4">
        {ata.contexto && (
          <div>
            ...
            <p className="mt-1 text-sm text-preto/80">{ata.contexto}</p>
          </div>
        )}
        ...
        {participantesNomeados.length > 0 && (
          <div>
            ...
            <p className="mt-1 text-sm text-preto/80">
              {participantesNomeados.map((p) => p.nome).join(" · ")}
            </p>
          </div>
        )}
